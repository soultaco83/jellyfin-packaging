...
# Function to setup PostgreSQL via JPVenson Pgsql plugin
setup_postgres() {
    # Only proceed if JELLYFIN_POSTGRES_ENABLED is set to true
    if [ "${JELLYFIN_POSTGRES_ENABLED}" != "true" ]; then
        echo "$(date '+%H:%M:%S') - PostgreSQL not enabled (JELLYFIN_POSTGRES_ENABLED != true)"
        return 0
    fi

    echo "$(date '+%H:%M:%S') - PostgreSQL enabled. Setting up JPVenson Pgsql plugin..."
    ...
    if [ -n "$missing_vars" ]; then
        echo "$(date '+%H:%M:%S') - ERROR: Missing required PostgreSQL environment variables:${missing_vars}"
        return 1
    fi

    local PLUGIN_DIR="/config/plugins/PostgreSQL"
    local DB_CONFIG_DIR="/config/config"
    local DB_CONFIG_FILE="${DB_CONFIG_DIR}/database.xml"
    local SQLITE_DB="/config/data/jellyfin.db"
    local SQLITE_DB_MIGRATED="/config/data/jellyfin.db.pgsql"

    # 1. Install plugin DLL from pre-built assets
    echo "$(date '+%H:%M:%S') - Installing PostgreSQL plugin DLL..."
    if [ -d "$PLUGIN_DIR" ]; then
        rm -rf "$PLUGIN_DIR"
    fi
    mkdir -p "$PLUGIN_DIR"
    if [ -d "/jellyfin-pgsql/plugin" ]; then
        cp -r /jellyfin-pgsql/plugin/* "$PLUGIN_DIR/"
        echo "$(date '+%H:%M:%S') - PostgreSQL plugin installed to $PLUGIN_DIR"
    else
        echo "$(date '+%H:%M:%S') - WARNING: Pgsql plugin assets not found at /jellyfin-pgsql/plugin. Plugin may not load."
    fi

    # 2. Build connection string and write database.xml
    echo "$(date '+%H:%M:%S') - Writing database.xml with PostgreSQL connection..."
    mkdir -p "$DB_CONFIG_DIR"

    local CONN_STR="Password=${POSTGRES_PASSWORD};User ID=${POSTGRES_USER};Host=${POSTGRES_HOST};Port=${POSTGRES_PORT};Database=${POSTGRES_DB}"

    # Append optional SSL parameters
    if [ -n "${POSTGRES_SSLMODE:-}" ]; then
        CONN_STR="${CONN_STR};SSL Mode=${POSTGRES_SSLMODE}"
    fi
    if [ -n "${POSTGRES_TRUSTSERVERCERTIFICATE:-}" ]; then
        CONN_STR="${CONN_STR};Trust Server Certificate=${POSTGRES_TRUSTSERVERCERTIFICATE}"
    fi

    # Write database.xml from template
    if [ -f "/jellyfin-pgsql/database.xml" ]; then
        cp /jellyfin-pgsql/database.xml "$DB_CONFIG_FILE"
        # Update the ConnectionString value using sed
        sed -i "s|<ConnectionString></ConnectionString>|<ConnectionString>${CONN_STR}</ConnectionString>|" "$DB_CONFIG_FILE"
        echo "$(date '+%H:%M:%S') - database.xml written with connection string"
    else
        echo "$(date '+%H:%M:%S') - WARNING: database.xml template not found at /jellyfin-pgsql/database.xml. Writing minimal config..."
        cat > "$DB_CONFIG_FILE" << XMLEOF
...
<DatabaseConfigurationOptions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <CustomProviderOptions>
    ...
    <ConnectionString>${CONN_STR}</ConnectionString>
  </CustomProviderOptions>
...
XMLEOF
    ...
    # 3. Migrate from SQLite if present and not yet migrated
    if [ -f "$SQLITE_DB" ] && [ ! -f "$SQLITE_DB_MIGRATED" ]; then
        echo "$(date '+%H:%M:%S') - Existing SQLite database detected. Starting migration to PostgreSQL..."

        # Step A: Seed PostgreSQL database tables via Jellyfin's SeedSystem mode
        echo "$(date '+%H:%M:%S') - Seeding PostgreSQL database (Jellyfin --mode SeedSystem)..."
        if ! /jellyfin/jellyfin --mode SeedSystem 2>&1; then
            echo "$(date '+%H:%M:%S') - WARNING: SeedSystem returned non-zero. Migration may still continue..."
        fi

        # Step B: Run pgloader to transfer data
        if command -v pgloader >/dev/null 2>&1; then
            echo "$(date '+%H:%M:%S') - Running pgloader to migrate SQLite data to PostgreSQL..."
            local LOAD_FILE="/jellyfin-pgsql/jellyfindb.load"

            if [ -f "$LOAD_FILE" ]; then
                # Run pgloader with environment variables for the .load file template
                if POSTGRES_USER="$POSTGRES_USER" POSTGRES_PASSWORD="$POSTGRES_PASSWORD" \
                   POSTGRES_HOST="$POSTGRES_HOST" POSTGRES_PORT="$POSTGRES_PORT" \
                   POSTGRES_DB="$POSTGRES_DB" pgloader "$LOAD_FILE" 2>&1; then
                    echo "$(date '+%H:%M:%S') - pgloader migration completed successfully"
                    touch "$SQLITE_DB_MIGRATED"
                    echo "$(date '+%H:%M:%S') - SQLite database marked as migrated ($SQLITE_DB_MIGRATED)"
                else
                    echo "$(date '+%H:%M:%S') - ERROR: pgloader migration failed. Check the logs above."
                    return 1
                fi
            else
                echo "$(date '+%H:%M:%S') - ERROR: jellyfindb.load not found at $LOAD_FILE. Cannot migrate."
                return 1
            fi
        else
            echo "$(date '+%H:%M:%S') - ERROR: pgloader not found. Cannot migrate SQLite data."
            return 1
        fi
    elif [ -f "$SQLITE_DB_MIGRATED" ]; then
        echo "$(date '+%H:%M:%S') - SQLite database already migrated ($SQLITE_DB_MIGRATED exists). Skipping migration."
    else
        echo "$(date '+%H:%M:%S') - No existing SQLite database found. Fresh PostgreSQL start."
        # Seed the database on first run with empty DB
        echo "$(date '+%H:%M:%S') - Seeding fresh PostgreSQL database (Jellyfin --mode SeedSystem)..."
        if ! /jellyfin/jellyfin --mode SeedSystem 2>&1; then
            echo "$(date '+%H:%M:%S') - WARNING: SeedSystem returned non-zero. This may be expected for a fresh DB."
        fi
    fi
...
# Setup PostgreSQL if enabled (must run before Jellyfin starts because it may seed/migrate the DB)
echo "$(date '+%H:%M:%S') - Checking for PostgreSQL setup..."
if ! setup_postgres; then
    echo "$(date '+%H:%M:%S') - CRITICAL: PostgreSQL setup failed. Check logs above."
    exit 1
fi
